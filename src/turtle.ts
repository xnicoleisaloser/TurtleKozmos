...
export class Inventory {
  static parseInventory(inventoryString?: string) {
    if (inventoryString) {
      ...
      const inventoryJson = JSON.parse('{ "inventory":' + JSON.stringify(inventoryString).toString() + "}")[
        "inventory"
      ];
      const inventory: InventorySlot[] = [];

      for (const slot of inventoryJson) {
        const inventorySlot = new InventorySlot(slot["name"], slot["count"]);
...
export class Turtle {
  ...
  constructor(turtleId: string, connection: ws) {
    ...
    this.turtleFuncCall = (func, args, module) => {
      this.connection.send(
        JSON.stringify({
          command: "callFunc",
          function: func,
          arguments: args.toString().replace("[", "").replace("]", ""),
          module: module,
        })
      );
    };
    ...
    this.turn = (rotations: number, direction: "right" | "left") => {
      const directionString = direction[0].toUpperCase() + direction.slice(1);
