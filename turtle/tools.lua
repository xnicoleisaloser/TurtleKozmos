...
function tools.inspect(args)
    ...
    if direction == "forward" then
        ...
        if has_block then return data["name"] else return "minecraft:air" end
    end
...
function tools.createName()
    local adjectives = { "good", "new", "first", "last", "long", "great", "little", "own", "other", "old", "right", "big", "high", "different", "small", "large", "next", "early", "young", "important", "few", "public", "bad", "same", "able" }
    local names = { "Zeus", "Hera", "Poseidon", "Demeter", "Ares", "Athena", "Apollo", "Artemis", "Hephaestus", "Aphrodite", "Hermes", "Dionysus", "Hades", "Hypnos", "Nike", "Janus", "Nemesis", "Iris", "Hecate", "Tyche"}
...
function tools.turn(rotations, direction) 
    if direction == "right" then for _ = 1, rotations, 1 do turtle.turnRight() end end
...
function tools.dig(args)
    ...
    if direction == "left" then turtle.turnLeft(); turtle.dig(); turtle.turnRight() end
    if direction == "right" then turtle.turnRight(); turtle.dig(); turtle.turnLeft() end
    if direction == "forward" then turtle.dig() end
    if direction == "backward" then tools.turn(2, "right"); turtle.dig(); tools.turn(2, "left") end
end
...
function tools.place(args)
    ...
    if direction == "up" then turtle.placeUp() end
    if direction == "down" then turtle.placeDown() end
    if direction == "left" then turtle.turnLeft(); turtle.place(); turtle.turnRight() end
    if direction == "right" then turtle.turnRight(); turtle.place(); turtle.turnLeft() end
    if direction == "forward" then turtle.place() end
    if direction == "backward" then tools.turn(2, "right"); turtle.place(); tools.turn(2, "left") end
end
...
function tools.place(args)
    ...
    if direction == "up" then turtle.drop(count) end
    if direction == "down" then turtle.dropDown(count) end
    if direction == "left" then turtle.turnLeft(); turtle.drop(count); turtle.turnRight() end
    if direction == "right" then turtle.turnRight(); turtle.drop(count); turtle.turnLeft() end
    if direction == "forward" then turtle.drop(count) end
    if direction == "backward" then tools.turn(2, "right"); turtle.drop(count); tools.turn(2, "left") end
end
